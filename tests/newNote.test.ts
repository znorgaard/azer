...
import { TABLE_SCHEMA } from "../src/schema/defaultTypes";
import { FakePorts } from "./fakes";
import { NPC_SCHEMA } from "./schemaFixtures";

describe("createTypedNote", () => {
  it("creates the folder, file, frontmatter, body, and opens it", async () => {
    const ports = new FakePorts();
    const handle = await createTypedNote(ports, NPC_SCHEMA, "Sildar Hallwinter", "NPCs");
    ...
    expect(file?.body).toBe(NPC_SCHEMA.bodyTemplate);
  ...
  it("does not recreate an existing folder", async () => {
    ...
    await createTypedNote(ports, NPC_SCHEMA, "Toblen", "NPCs");
  ...
  it("refuses to overwrite an existing note", async () => {
    ...
    await expect(
      createTypedNote(ports, NPC_SCHEMA, "Sildar Hallwinter", "NPCs"),
    ).rejects.toThrow(/already exists/);
  });

  it("rejects a blank name", async () => {
    const ports = new FakePorts();
    await expect(createTypedNote(ports, NPC_SCHEMA, "   ", "NPCs")).rejects.toThrow(
      /name is required/,
    );
  });
  ...
  it("uses a body override when provided instead of the schema template", async () => {
    ...
    await createTypedNote(ports, TABLE_SCHEMA, "Ambushes", "Tables", body);
  ...
  it("writes an empty-string body override literally (??, not || → no template fallback)", async () => {
    const ports = new FakePorts();
    await createTypedNote(ports, NPC_SCHEMA, "Ghost", "NPCs", "");
